...
if game:GetService("UserInputService").VREnabled then 
	getParent = Parents[3]
	loadstring(game:HttpGet("https://raw.githubusercontent.com/FunaSense/FunaSense/main/nexus_support.lua"))()
end
...
Stand("FunaSesne is in development.", "Please join the discord for development updates.", "You can get the discord invite by using the .discord command.", "", "", "Continue", true)
while not RunDude do wait() end

wait(.5)

loadstring(game:HttpGet("https://raw.githubusercontent.com/FunaSense/FunaSense/main/Version.lua",true))()
if Current and Current.Version ~= ver then
	Stand("FunaSesne is not up to par!","FunaSesne cannot run.","","Your version: "..ver,"Current version: "..Current.Version,"Run Anyway",true)
...
if isfile then
	if not isfile("FunaSesne.lua") then
...
if AntiCheat.HideParentInExploit then
	...
	elseif get_hidden_gui then
		...
		Stand("Your exploit does not support syn.protect_gui/get_hidden_gui!","FunaSesne cannot run.","Explanation;","This game has an anti-cheat and our way of stopping it is through syn.protect_gui/get_hidden_gui.","","Run Anyway",true)
		repeat wait() until RunDude == true
	end
end
...
if AntiCheat.RemoveScripts then
	...
	Stand("FunaSesne has to delete local scripts!","Your game may not work.","Explanation;","This game has an anticheat and our way of solving it is removing scripts.","","Run Anyway",true)
	repeat wait() until RunDude == true
end
...
opxviewertitle.Text = "FunaSesne OUTPUT-LONGER"
...
logsviewertitle.Text = "FunaSesne LOGS VIEWER"
...
Save2.MouseButton1Down:Connect(function()
	...
	writefile("FunaSesne Logs "..math.random(1000)..".txt",tostring(writestats2))
end)
...
function opx(f,text)
	if not ChatBind then
		...
		if f == "*" then
			sayremote:FireServer("[FunaSesne] [*] "..text, "All")
		elseif f == "-" then
			sayremote:FireServer("[FunaSesne] [-] "..text, "All")
		end
	end
...
ManageTitleEnter.Text = "FunaSesne EnterCMDs MANAGER"
...
function useCommand.advertise()
	...
	while cmdAdvertise do
		local str = "Get FunaSesne today! Best admin GUI, supports paid executors! type in FunaSesne on google!"
	...
	opx("-","Now advertising FunaSesne (Thank you!)")
end

function useCommand.unadvertise()
	cmdAdvertise = false
	opx("-","Stopped advertising FunaSesne (Thank you!)")
end

function useCommand.version()
	opx("-","FunaSesne is currently on "..ver)
end
...
function useCommand.playerstalker()
	loadstring(game:HttpGet("https://raw.githubusercontent.com/FunaSense/FunaSense/main/PlayerStalkerV2.lua",true))()
	opx("-","Loaded PlayerStalker")
end

function useCommand.badger()
	loadstring(game:HttpGet("https://raw.githubusercontent.com/FunaSense/FunaSense/main/badger.lua",true))()
